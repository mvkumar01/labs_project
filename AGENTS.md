...
├── eod_recovery.py              One-time forced close of any open positions
│
├── auth/
│   ├── session_manager.py       KiteConnect singleton (auto-reloads token from JSON)
...
│   │   ├── data_loader.py          CSV → DataFrame for spot + options; warmup backfill helper
...
│   │   ├── resampler.py            get_resampled_data(df_1min, tf, now) → broker-style completed bars
│   │   └── strategy_runner.py      Main 60s loop: leg-based or classic path per bot; EOD square-off + warmup
│   │
...
- `auth/session_manager.py` now auto-reloads `zerodha_token.json` when it changes, so long-running tasks can pick up refreshed tokens without a restart.
- `labs/engine/strategy_runner.py` now keeps running outside market hours, performs EOD square-off after cutoff, and logs warmup status for SMA50-based gates.
- `labs/engine/data_loader.py` now supports prior-session 1-minute warmup backfill for SMA50 readiness from market open.
- `labs/engine/resampler.py` now uses broker-style left-labeled 5-minute resampling for completed-bar evaluation.
