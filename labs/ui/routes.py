...
def live_strategy():
    ...
    try:
        ...
        if active_live_tab in {"alpha_v211b", "alpha_v212", "alpha_v212b10",
                               "alpha_v214", "alpha_v214c", "alpha_cpr"}:
...
@labs_bp.route("/api/alpha_cpr/backfill", methods=["POST"])
def alpha_cpr_backfill():
    """Replay up to `limit` pending Alpha-CPR days (default from 2026-06-01)
    into alpha_cpr_daily/_trades, reusing the same per-day champion ranges as
    v2.12/v2.13 — only the exits differ. Bounded per call so a PA web request
