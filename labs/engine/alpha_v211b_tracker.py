"""Alpha v2.11 champion replay B paper tracker.

This ledger runs the canonical v2.11 decision and exit stack with one isolated
entry rule: PC50 CALL decisions stay flat. PC50 PUT and every PC250/PC400
decision are unchanged. Each accepted segment buys one nearest-expiry NIFTY
option 200 points ITM at executable ask and sells at executable bid.
...
STRATEGY_VERSION = "alpha_v2.11_champion_replay_b_no_pc50_call_itm200_bidask"


class AlphaV211BInputError(RuntimeError):
...
def _ensure_tables(conn: sqlite3.Connection) -> None:
    conn.executescript(
        """
        CREATE TABLE IF NOT EXISTS alpha_v211b_daily (
        ...
        CREATE TABLE IF NOT EXISTS alpha_v211b_trades (
...
def replay_v211b(trade_date: str, override: dict | None = None) -> dict:
    try:
        ...
        raise AlphaV211BInputError(str(exc)) from exc
    context = dict(replay.get("context") or {})
    context.update(
        {
            "strategy_version": "Alpha 2.11 - champion replay (B)",
...
def _save(
    ...
    *,
    ...
    conn.execute("DELETE FROM alpha_v211b_trades WHERE trade_date=?", (trade_date,))
    for seq, trade in enumerate(trades, 1):
        conn.execute(
            "INSERT INTO alpha_v211b_trades "
    ...
    conn.execute(
        ...
        (
            ...
            STRATEGY_VERSION, json.dumps(replay.get("context"), sort_keys=True), now,
        ),
    )
    if commit:
        conn.commit()
...
def run_day(
    ...
    *,
    ...
    replay = replay_v211b(trade_date, override)
    expiry_code = None
    trades: list[dict] = []
    if replay["segments"]:
        try:
    ...
    if require_all_quotes and unavailable:
        ...
        raise AlphaV211BInputError(
            f"Alpha v2.11 champion replay B pricing incomplete for {trade_date}: "
