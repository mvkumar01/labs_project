...
class AlphaV214InputError(RuntimeError):
...
def _ensure_tables(conn: sqlite3.Connection) -> None:
    conn.executescript(
        """
        CREATE TABLE IF NOT EXISTS alpha_v214_daily (
        ...
        CREATE TABLE IF NOT EXISTS alpha_v214_trades (
...
def replay_v214(trade_date: str, override: dict | None = None) -> dict:
    try:
        replay = replay_v212(
            trade_date,
            override,
...
def _save(
    ...
    *,
    ...
    conn.execute("DELETE FROM alpha_v214_trades WHERE trade_date=?", (trade_date,))
    for seq, trade in enumerate(trades, 1):
        conn.execute(
            "INSERT INTO alpha_v214_trades "
...
def run_day(
    ...
    *,
    ...
    replay = replay_v214(trade_date, override)
    expiry_code = None
    trades: list[dict] = []
    if replay["segments"]:
        try:
