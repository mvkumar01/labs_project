"""Bounded historical backfill for the Alpha v2.14 C paper book.

v2.14 C is v2.14 B with a Renko 30 (classic reversal) overlay.

Uses the same per-day champion ranges as every sibling alpha book (the audited
June manifest plus later v2.11 paper contexts), so v2.14 C covers exactly the days
...
segment aborts the batch and leaves existing rows untouched.
    ...
    python3 -m labs.engine.alpha_v214c_backfill            # resume to done
    python3 -m labs.engine.alpha_v214c_backfill --rebuild  # recompute all
"""
from __future__ import annotations

from labs.engine.alpha_v214c_tracker import _ensure_tables, run_day
...
def _pending(start_date: str, end_date: str | None, conn) -> tuple[list, dict]:
    ...
    ranges = _historical_ranges(start_date, end_date)
        ...
        row[0] for row in conn.execute("SELECT trade_date FROM alpha_v214c_daily")
    }
...
def _run_to_completion(start: str, end: str | None, rebuild: bool) -> int:
    ...
    if rebuild:
        ...
        try:
            _ensure_tables(conn)
            conn.execute("DELETE FROM alpha_v214c_trades WHERE trade_date >= ?",
                         (start,))
            conn.execute("DELETE FROM alpha_v214c_daily WHERE trade_date >= ?",
                         (start,))
            conn.commit()
        finally:
            conn.close()
